...
import { handleGetMyAssignedTickets } from "../../controllers/ticketController";
...
function isOverdue(ticket) {
  ...
  return new Date(ticket.dueDate) < new Date() && !["RESOLVED", "CLOSED"].includes(ticket.status);
}

function MyAssignedTicketsPage() {
  ...
  // Mobile: track whether we're showing the detail panel as a full screen overlay
  const [mobilePanelOpen, setMobilePanelOpen] = useState(false);

  const fetchTickets = useCallback(async (showLoading = true) => {
    try {
      if (showLoading) setLoading(true);
      const data = await handleGetMyAssignedTickets();
    ...
    <>
      <style>{`
        ...
        [data-theme="dark"] {
          ...
          --tk-row-hover:    rgba(255,255,255,0.03);
          --tk-row-border:   rgba(255,255,255,0.04);
          --tk-row-sel-bg:   rgba(99,102,241,0.07);
          --tk-row-sel-pip:  #6366f1;
          --tk-header-bg:    rgba(255,255,255,0.025);
          --tk-text:         #e8e8f0;
          --tk-text2:        #9898b8;
          --tk-text3:        #50506a;
          --tk-input-bg:     rgba(255,255,255,0.05);
          --tk-input-border: rgba(255,255,255,0.1);
          --tk-filter-bg:    rgba(255,255,255,0.03);
          --tk-select-bg:    rgba(255,255,255,0.05);
          --tk-pill-bg:      rgba(255,255,255,0.05);
          --tk-pill-border:  rgba(255,255,255,0.08);
          --tk-empty-border: rgba(255,255,255,0.05);
          --tk-stat-bg:      rgba(255,255,255,0.03);
          --tk-overdue-bg:   rgba(239,68,68,0.08);
          --tk-overdue-border: rgba(239,68,68,0.2);
        }
        [data-theme="light"] {
          --tk-bg:           #f7f7fb;
          --tk-card:         #ffffff;
          --tk-border:       #e5e7eb;
          --tk-row-hover:    #fafafa;
          --tk-row-border:   #f5f5f8;
          --tk-row-sel-bg:   rgba(99,102,241,0.05);
          --tk-row-sel-pip:  #6366f1;
          --tk-header-bg:    #f9fafb;
          --tk-text:         #111827;
          --tk-text2:        #4b5563;
          --tk-text3:        #9ca3af;
          --tk-input-bg:     #ffffff;
          --tk-input-border: #e5e7eb;
          --tk-filter-bg:    #ffffff;
          --tk-select-bg:    #ffffff;
          --tk-pill-bg:      #f3f4f6;
          --tk-pill-border:  #e5e7eb;
          --tk-empty-border: #e5e7eb;
          --tk-stat-bg:      #f9fafb;
          --tk-overdue-bg:   rgba(239,68,68,0.05);
          --tk-overdue-border: rgba(239,68,68,0.15);
        }
        ...
        .tk-row.sel {
          background: var(--tk-row-sel-bg) !important;
          border-left-color: var(--tk-row-sel-pip) !important;
        }
        .tk-row.overdue-row { border-left-color: rgba(239,68,68,0.4); }
        ...
        .tk-card-row.overdue-row { border-left-color: rgba(239,68,68,0.4); }

        .tk-select {
          height: 34px; padding: 0 28px 0 10px;
          border-radius: 8px; border: 1px solid var(--tk-input-border);
          background: var(--tk-select-bg); color: var(--tk-text);
        ...
        @media (max-width: 480px) {
          .tk-stat-strip { gap: 6px; }
          .tk-stat-strip > div { padding: 7px 10px !important; }
        }
        ...
        /* ── Detail panel: side panel on desktop, hidden on mobile (uses overlay) ── */
        .tk-side-panel {
          display: flex; flex-shrink: 0;
          animation: tk-panelIn 0.25s cubic-bezier(0.16,1,0.3,1);
        }
        @media (max-width: 1023px) {
          .tk-side-panel { display: none; }
        }
        ...
        @media (max-width: 640px) {
          .tk-list-pad { padding: 16px 14px; }
        }
      `}</style>

      {/* ── Mobile full-screen detail panel overlay (< 1024px) ── */}
        ...
        <div className="tk-list-pad">
          <div style={{ animation: "tk-fadeUp 0.2s ease" }}>
            ...
            <div style={{ marginBottom: 20 }}>
              <h2 style={{ fontSize: 20, fontWeight: 800, color: "var(--tk-text)", margin: 0, letterSpacing: "-0.02em" }}>Assigned to Me</h2>
              <p style={{ fontSize: 13, color: "var(--tk-text3)", margin: "4px 0 0" }}>
                Tickets assigned for you to handle · {tickets.length} total
                {overdueCount > 0 && (
                  <span style={{ marginLeft: 8, padding: "2px 7px", borderRadius: 99, background: "rgba(239,68,68,0.1)", color: "#ef4444", fontSize: 11, fontWeight: 700 }}>
            ...
            {/* Stat strip */}
            {tickets.length > 0 && (
              <div className="tk-stat-strip">
                {[
                  { key: "ALL", label: "Total", count: tickets.length, color: "#6366f1" },
                  { key: "IN_PROGRESS", label: "In Progress", count: statusCounts["IN_PROGRESS"] || 0, color: "#ca8a04" },
                  { key: "OPEN", label: "Open", count: (statusCounts["OPEN"] || 0) + (statusCounts["ASSIGNED"] || 0), color: "#3b82f6" },
                  { key: "RESOLVED", label: "Resolved", count: statusCounts["RESOLVED"] || 0, color: "#16a34a" },
                ].map(({ key, label, count, color }) => (
                  <div key={key} style={{
                    display: "flex", alignItems: "center", gap: 10,
                    padding: "8px 14px", borderRadius: 10, border: "1px solid var(--tk-border)",
                    background: "var(--tk-stat-bg)", fontFamily: "inherit",
                    transition: "all 0.15s", flexShrink: 0,
                  }}>
                    <div style={{ width: 6, height: 6, borderRadius: "50%", background: color }} />
                    <div style={{ textAlign: "left" }}>
                      <div style={{ fontSize: 17, fontWeight: 800, color, letterSpacing: "-0.03em", lineHeight: 1 }}>{count}</div>
                      <div style={{ fontSize: 9.5, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--tk-text3)", marginTop: 1 }}>{label}</div>
                    </div>
                  </div>
                ))}
                {overdueCount > 0 && (
                  <div style={{
                    ...
                    border: "1px solid var(--tk-overdue-border)",
                    background: "var(--tk-overdue-bg)", flexShrink: 0,
                  }}>
                    <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#ef4444" }} />
                    <div>
                      <div style={{ fontSize: 17, fontWeight: 800, color: "#ef4444", letterSpacing: "-0.03em", lineHeight: 1 }}>{overdueCount}</div>
                      <div style={{ fontSize: 9.5, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.07em", color: "#ef4444", marginTop: 1, opacity: 0.7 }}>Overdue</div>
                    </div>
                  </div>
                )}
              </div>
            ...
            {tickets.length > 0 && (
              <div className="tk-filter-bar">
                ...
                {hasFilters && (
                  <button className="tk-btn-ghost" onClick={() => { setStatusFilter("ALL"); setPriorityFilter("ALL"); }}
            ...
            {filteredTickets.length === 0 ? (
              <div style={{
                ...
                <p style={{ fontSize: 14, fontWeight: 600, color: "var(--tk-text2)", margin: 0 }}>
                  {hasFilters ? "No tickets match your filters" : "No tickets assigned to you"}
                </p>
                <p style={{ fontSize: 13, color: "var(--tk-text3)", margin: 0 }}>
                  {hasFilters ? "Try adjusting or resetting the filters above." : "Tickets assigned to you will appear here."}
                </p>
              </div>
            ) : (
              <>
                ...
                <div className="tk-table-view">
                  <div style={{ background: "var(--tk-card)", borderRadius: 14, border: "1px solid var(--tk-border)", overflow: "hidden" }}>
                    <div className="tk-row" style={{
                      gridTemplateColumns: selectedTicketId ? "120px 1fr 110px 90px 80px" : "140px 1fr 110px 90px 140px 80px",
                      ...
                      {!selectedTicketId && <span className="tk-col-label">Raised By</span>}
                    ...
                    {filteredTickets.map((t) => {
                      ...
                      return (
                        <div key={t._id}
                          className={`tk-row${isSelected ? " sel" : ""}${overdue && !isSelected ? " overdue-row" : ""}`}
                          onClick={() => handleSelectTicket(t.ticketId)}
                          style={{
                            gridTemplateColumns: selectedTicketId ? "120px 1fr 110px 90px 80px" : "140px 1fr 110px 90px 140px 80px",
                            gap: 12, padding: "13px 18px 13px 18px",
                          }}>
                          ...
                          <div style={{ minWidth: 0 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                              <span style={{ fontSize: 13, fontWeight: 600, color: "var(--tk-text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.title}</span>
                              {overdue && <span style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", padding: "1px 5px", borderRadius: 4, background: "rgba(239,68,68,0.12)", color: "#ef4444", flexShrink: 0 }}>Overdue</span>}
                            </div>
                          ...
                          {!selectedTicketId && (
                            <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12, color: "var(--tk-text2)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                              <IcUserRaised />{t.raisedBy?.name || <span style={{ color: "var(--tk-text3)", fontStyle: "italic" }}>Unknown</span>}
                            </span>
                          )}
                          <span style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                            <div>
                              ...
                              {t.dueDate && (
                                <span style={{ display: "flex", alignItems: "center", gap: 3, fontSize: 10, color: overdue ? "#ef4444" : "var(--tk-text3)", marginTop: 2 }}>
                                  <IcClock />Due {new Date(t.dueDate).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                                </span>
                              )}
                            </div>
                ...
                <div className="tk-cards-view">
                  <div style={{ background: "var(--tk-card)", borderRadius: 14, border: "1px solid var(--tk-border)", overflow: "hidden" }}>
                    {filteredTickets.map((t) => {
                      ...
                      return (
                        <div key={t._id}
                          className={`tk-card-row${isSelected ? " sel" : ""}${overdue && !isSelected ? " overdue-row" : ""}`}
                          onClick={() => handleSelectTicket(t.ticketId)}>
                          {/* Top row: ID + badges */}
                          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                            ...
                            {overdue && <span style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", padding: "1px 5px", borderRadius: 4, background: "rgba(239,68,68,0.12)", color: "#ef4444" }}>Overdue</span>}
                          </div>
                          {/* Title */}
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                            {t.raisedBy?.name && (
                              <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, color: "var(--tk-text2)" }}>
                                <IcUserRaised />{t.raisedBy.name}
                              </span>
                            )}
                            <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, color: "var(--tk-text3)" }}>
                              <IcCalendar />{new Date(t.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                            ...
                            {t.dueDate && (
                              <span style={{ display: "inline-flex", alignItems: "center", gap: 3, fontSize: 11, color: overdue ? "#ef4444" : "var(--tk-text3)" }}>
                                <IcClock />Due {new Date(t.dueDate).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                              </span>
                            )}
